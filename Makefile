...
ifeq ($(DOCKER_IMAGE),)
	DOCKER_IMAGE=harness-payload-generator
endif
ifeq ($(DOCKER_TAG),)
	DOCKER_TAG=latest
endif
...
WORKDIR=/harness
DOCKER_RUN=${DOCKER_COMMAND} run --rm -it ${DOCKER_ENV} -v ${PROJECT_DIR}:/${WORKDIR} ${DOCKER_MOUNTS} -w ${WORKDIR}/${TEMPLATE_DIR} $(ENTRYPOINT) ${TERRAFORM_IMAGE}:${TERRAFORM_VERSION}
